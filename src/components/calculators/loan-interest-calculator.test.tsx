...
describe("LoanInterestCalculator", () => {
  ...
  it("shows the estimated result while typing without submitting", async () => {
    ...
    expect(screen.getByText("월 예상 이자")).toBeInTheDocument();
    expect(screen.getByText("350,000원")).toBeInTheDocument();
    expect(screen.getByText("4,200,000원")).toBeInTheDocument();
  });
  ...
  it("shows next step action buttons after the estimated result", async () => {
    ...
    expect(screen.getByRole("link", { name: "실제 금리 확인 →" })).toHaveAttribute("href", "https://fine.fss.or.kr/");
    expect(screen.getByRole("link", { name: "다른 계산기 보기 →" })).toHaveAttribute("href", "/#calculators");
  });
  ...
  it("offers mobile-friendly amount and rate presets", async () => {
    ...
    expect(screen.getByText("666,667원")).toBeInTheDocument();
  });
